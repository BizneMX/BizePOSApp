...
class BizneSearchField extends StatefulWidget {
  final TextEditingController controller;
  final String hint;
  ...
  @override
  State<BizneSearchField> createState() => BizneSearchFieldState();
}

class BizneSearchFieldState extends State<BizneSearchField> {
  @override
  Widget build(BuildContext context) {
    return TextField(
      controller: widget.controller,
      ...
      onTapOutside: (_) => widget.onSubmitted,
      onSubmitted: (_) => widget.onSubmitted(),
      decoration: InputDecoration(
        hintText: widget.hint,
        hintStyle: TextStyle(fontFamily: 'Quicksand', fontSize: 10.sp),
        // prefixIcon: Icon(Icons.search, color: AppThemes().secondary),
        suffixIcon: InkWell(
        ...
        fillColor: AppThemes().whiteInputs,
        border: OutlineInputBorder(
          borderSide: BorderSide(color: AppThemes().primary, width: 1),
          borderRadius: AppThemes().borderRadius,
        ),
        focusedBorder: OutlineInputBorder(
          borderSide: BorderSide(color: AppThemes().primary, width: 2),
          borderRadius: AppThemes().borderRadius,
        ),
        enabledBorder: OutlineInputBorder(
          borderRadius: AppThemes().borderRadius,
...
class BizneScheduleField extends StatefulWidget {
  final String initial;
  final void Function(String) onChanged;
  const BizneScheduleField(
      {super.key, required this.initial, required this.onChanged});

  @override
  State<BizneScheduleField> createState() => _BizneScheduleFieldState();
}

class _BizneScheduleFieldState extends State<BizneScheduleField> {
  ...
  void initState() {
    ...
    lastText = widget.initial;

    controller.addListener(() {
      if (lastText.length < controller.text.length &&
          controller.text.length == 3) {
        controller.text =
            '${controller.text.substring(0, 2)}:${controller.text[2]}';
      }
